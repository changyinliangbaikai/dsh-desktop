/** Verify final ASAR resources, native shell provenance, and an isolated Host. */
...
const requireBuilder = createRequire(requireApp.resolve('app-builder-lib/package.json'));
...
try {
  const output = execFileSync(join(unpacked, 'Harness Desktop Intranet.exe'), [
    '--expose-internals', join(repository, 'scripts/native-runtime-smoke.mjs'), join(archive, 'dsh'),
  ], {
    timeout: 180_000, maxBuffer: 16 * 1024 * 1024, encoding: 'utf8',
